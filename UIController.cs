		...
		if(turn.isPlayerTurn()){
			...
			while (i < Input.touchCount) {
				if (Input.GetTouch(i).phase == TouchPhase.Began){
					...
					bool hit = Physics.Raycast(Camera.main.ScreenToWorldPoint(Input.GetTouch(i).position), fwd, out rayTarget);
					...
					if(hit){
						 if(rayTarget.collider != null){
							 ...
							 if(type.getType() == Target.Type.NPC){
							 ...
							 if(type.getType() == Target.Type.UI){
								 ...
								 if(isSelecting){
										 ...
										 if(this.highlighted.Contains(new Point((int)position.x, (int)position.z))){
											 print("hiiii");
